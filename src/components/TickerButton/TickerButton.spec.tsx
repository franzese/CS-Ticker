...
describe('The TickerButton component', () => {
    it('uses a native <button/> with nested text to render', () => {
        const { container } = render(<TickerButton buttonType={ButtonTypes.button}>test</TickerButton>);
    ...
    it('should not trigger handleClick but not onClick prop if no onClick method is passed', () => {
        const onClickMock = jest.fn().mockImplementation(() => {});
        const { getByTestId } = render(<TickerButton buttonType={ButtonTypes.button}>test</TickerButton>);
        const testTickerButton = getByTestId('TickerButton');
    ...
    it('should apply the normal button styles based on ButtonType.button prop', () => {
        const { getByTestId } = render(<TickerButton buttonType={ButtonTypes.button}>testPrimary</TickerButton>);
        const testTickerButton = getByTestId('TickerButton');

        expect(testTickerButton.className).toBe(ButtonTypes.button);
    });

    it('should apply the dropdown button styles based on ButtonTypes.dropdown prop', () => {
        const { getByTestId } = render(<TickerButton buttonType={ButtonTypes.dropdown}>testPrimary</TickerButton>);
        const testTickerButton = getByTestId('TickerButton');

        expect(testTickerButton.className).toBe('tickerButtonDropdown');
    });
    ...
    it('should apply the default buttonType when no buttonType is passed', () => {
        ...
        expect(testTickerButton.className).toBe('tickerButton');
    });
